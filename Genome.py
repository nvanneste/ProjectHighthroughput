...
with open ('Harvard.csv') as Harvardfile:
	...
	for row in reader:
	   idlist2.append(row['userid'])#lijst maken van alle id's die aanwezig zijn in het harvard project
...
f = open('GenomeProject.csv', mode = 'a')

header = ("{},{},{},{}".format(user,name,download,filename))
f.write(str(header))

for i in range(0, len(uniek), 1): #de lengte van de idlist wordt er naar overeenkomende gezocht eerste idlist is kleinste dus meer overlap zou niet kunnen
	...
	#id uit de lijst die nooit ook op de eerste plaats zal komen als in de api
		userid = data["results"][j]["user"]["id"] #userids bekijken van de genome api
		if uniek[i] == userid and uniek[i] == userid and 'vcf.gz' in data["results"][j]["basename"]: # zoeken naar vcf.gz en dezelfde userid als in de uniek lijst
			...
			f.write("\n")
			f.write(str("{}, {}, {}, {}".format(userid, username, downloadurl, basename)))
